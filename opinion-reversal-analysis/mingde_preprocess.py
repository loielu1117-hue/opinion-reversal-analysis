...
# 读取所有李明德事件的CSV文件
files = glob.glob("zhihu_data_李明德1/*.csv") + glob.glob("zhihu_data_李明德2/*.csv") + glob.glob("zhihu_data_李明德3/*.csv")+ glob.glob("zhihu_data_李明德4/*.csv")+glob.glob("zhihu_data_李明德5/*.csv")
...
# 保存到单个文件，方便后续分析
df.to_csv('zhihu_limingde_cleaned.csv', index=False, encoding='utf-8-sig')
print("数据已保存")